...
export function AdminHeader() {
  const { counsellor, logout } = useAdminAuth()
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                  <Avatar className="h-8 w-8">
                    <AvatarImage src="/avatars/01.png" alt={counsellor?.name || "Counsellor"} />
                    <AvatarFallback>{counsellor?.name ? getInitials(counsellor.name) : "CN"}</AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-56" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-medium leading-none">{counsellor?.name || "Counsellor"}</p>
